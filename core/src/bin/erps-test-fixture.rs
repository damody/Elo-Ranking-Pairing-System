...
async fn main() -> anyhow::Result<()> {
    ...
    loop {
        tokio::select! {value=controls.message()=>{let Some(control)=value? else{break};if let Some(pb::erps_control::Message::Launch(launch))=control.message{tx.send(pb::ServerControl{server_id:server_id.clone(),generation:1,auth_token:"fixture-server".into(),message:Some(pb::server_control::Message::LaunchResult(pb::LaunchResult{match_id:launch.match_id,state:"ready".into(),endpoint:"127.0.0.1:7001".into(),connection_token:uuid::Uuid::new_v4().to_string(),reason:String::new()})),api:Some(api())}).await?;}} _=heartbeat.tick()=>{tx.send(pb::ServerControl{server_id:server_id.clone(),generation:1,auth_token:"fixture-server".into(),message:Some(pb::server_control::Message::Heartbeat(pb::Heartbeat{capacity_used:0,running_instances:0})),api:Some(api())}).await?;} _=tokio::signal::ctrl_c()=>break}
    }
    let _ = stop_tx.send(());
    Ok(())
}
